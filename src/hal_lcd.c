...
void lcd_init(void)
{
    ...
    ste_write(FLAG_CMD, 0x20);                  // Set voltage range
    ste_write(FLAG_CMD, LCD_CONTRAST);

    ste_write(FLAG_CMD, LCD_MODE | NORMAL);
    ste_write(FLAG_CMD, LCD_CHARGE_PUMP_ON);    // Enable charge pump
    ste_write(FLAG_CMD, LCD_MODE | ON);         // Display on
    ste_write(FLAG_CMD, 0x9F);
...
{
    ste_write(FLAG_CMD, LCD_CHANGE_COL_LSB | ( col & 0x0F));
    ste_write(FLAG_CMD, LCD_CHANGE_COL_MSB | ((col >> 4) & 0x07 ));
    ste_write(FLAG_CMD, LCD_CHANGE_ROW     | (row & 0x0F));
}
...
void lcd_normal(void)
{
    ...
    ste_write(FLAG_CMD, LCD_MODE|NORMAL);
    ste_write(FLAG_CMD, LCD_RAM_ADDR_VER_MODE);
}
